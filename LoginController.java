...
public class LoginController {
    ...
    private static class NomineeAppointeeData {
        List<PolicyRelation> nominees;
        List<PolicyRelation> appointees;
        
        NomineeAppointeeData(List<PolicyRelation> nominees, List<PolicyRelation> appointees) {
    ...
    public LoginResponseDto hello(@PathVariable("polId") String polId) {
        // String liDob = "1990-07-30";
        // String claimantDob = "1998-04-08";
        ...
        List<NomineeDto> nominees = mapNomineeDetails(nomineeAppointeeData.nominees);
        
        // STEP 8: Map appointee details (after OTP verification)
        List<AppointeeDto> appointees = mapAppointeeDetails(nomineeAppointeeData.appointees);
    ...
    private NomineeAppointeeData aggregateNomineesAndAppointees(List<PolicyDto> allPolicies) {
        List<PolicyRelation> nominees = new ArrayList<>();
        List<PolicyRelation> appointees = new ArrayList<>();
        ...
        for (PolicyDto policy : allPolicies) {
            ...
            extractNomineesFromPolicy(polPayload, nominees);
            
            // Extract appointees from policyClientRelationship
            extractAppointeesFromPolicy(polPayload, appointees, excludeRelatnTypes);
        }
        
        return new NomineeAppointeeData(nominees, appointees);
    }
        ...
        if (nomineeDetails != null && nomineeDetails.getNominees() != null) {
            for (NomineeDetailsDto.NomineeDto nominee : nomineeDetails.getNominees()) {
                ...
                if (nomineeClientID != null && !nomineeClientID.trim().isBlank()) {
                    ...
                    nominees.add(new PolicyRelation(polPayload.getPolicyNo(), nomineeRelationDto));
                }
            }
        }
    }
            ...
            for (PolicyClientRelationshipDto relation : polPayload.getPolicyClientRelationship()) {
                ...
                if (clientID != null && !clientID.trim().isBlank()) {
                    ...
                    if (!excludeRelatnTypes.contains(trimmedRelatnType)) {
                        appointees.add(new PolicyRelation(polPayload.getPolicyNo(), relation));
                    }
                }
            }
        }
     ...
     * Business rules: At least 1 Nominee is required, multiple unique nominees not allowed
     */
    private void validateNomineeAndAppointeeCount(NomineeAppointeeData data) {
        System.out.println("Nominee count: " + data.nominees.size());
        if (data.nominees.isEmpty()) {
            throw new RuntimeException("At least 1 Nominee is required");
        }
        
        // Check for unique nominee client IDs
        Set<String> uniqueNomineeClientIds = new HashSet<>();
        for (PolicyRelation nominee : data.nominees) {
            String clientId = nominee.relation.getClientID();
            if (clientId != null && !clientId.trim().isBlank()) {
                uniqueNomineeClientIds.add(clientId.trim());
            }
        }
        
        System.out.println("Unique Nominee Client IDs: " + uniqueNomineeClientIds);
        if (uniqueNomineeClientIds.size() > 1) {
            throw new RuntimeException("Multiple nominees not allowed");
        }
        
        // Check for unique appointee client IDs
        Set<String> uniqueAppointeeClientIds = new HashSet<>();
        for (PolicyRelation appointee : data.appointees) {
            String clientId = appointee.relation.getClientID();
            if (clientId != null && !clientId.trim().isBlank()) {
                uniqueAppointeeClientIds.add(clientId.trim());
            }
        }
        
        System.out.println("Unique Appointee Client IDs: " + uniqueAppointeeClientIds);
        System.out.println("Appointee count: " + data.appointees.size());
        if (uniqueAppointeeClientIds.size() > 1) {
     ...
     */
    private List<NomineeDto> mapNomineeDetails(List<PolicyRelation> nominees) {
        List<NomineeDto> allNomineeDetails = new ArrayList<>();
        
        for (PolicyRelation policyRelation : nominees) {
            String nomineeId = policyRelation.relation.getClientID();
            if (nomineeId != null) {
                ClientDto nomineeDetail = loginService.getClientDetails(nomineeId.trim());
                NomineeDto nomineeDto = buildNomineeDto(nomineeId.trim(), nomineeDetail, policyRelation);
     ...
     */
    private List<AppointeeDto> mapAppointeeDetails(List<PolicyRelation> appointees) {
        List<AppointeeDto> allAppointeeDetails = new ArrayList<>();
        
        for (PolicyRelation policyRelation : appointees) {
            String appointeeId = policyRelation.relation.getClientID();
            if (appointeeId != null) {
                ClientDto appointeeDetail = loginService.getClientDetails(appointeeId.trim());
                AppointeeDto appointeeDto = buildAppointeeDto(appointeeId.trim(), appointeeDetail, policyRelation);
